...
interface Student extends RowEntry{
    name: string,
    login: string
}
...
async function getStudents() {
    try {
        ...
        if (res.ok) {
            ...
            return responseBody as Student[];
        }
    } catch (e) {
        return null;
    }
...
export default async function Page() {
    ...
    if (students === null || students === undefined) {
        return (
            <ErrorCard title={"Error"} content={errorMessage}/>
        )
    } else {
        return (
                <Table<Student> rows={students} titles={["Id", "Student name", "login"]}/>
        );
    }
}
...
//             </td>
//         </tr>
//         {Array.from({length: 20}, (e, i) => i).map(s => <TableRow key={s}/>)}
//         </tbody>
//     </table>
// </div>